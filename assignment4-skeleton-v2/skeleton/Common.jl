...
function nlfilter(img, fun=median, m=3, n=3, border="replicate")
  ...
  # map windows
  img_filtered = ImageFiltering.mapwindow(fun_linearized, img, [m, n], border)
  return img_filtered
end
...
#
#---------------------------------------------------------
function computeharris(S_xx::Array{Float64,2},
                       S_yy::Array{Float64,2},
                       S_xy::Array{Float64,2},
                       sigma::Float64,
                       alpha::Float64)
...
#
#---------------------------------------------------------
function detect_interestpoints(img, sigma, fsize, threshold, boundary)
  ...
  px, py = nonmaxsupp(harris, threshold, boundary)
  return px, py
end
...
#
#---------------------------------------------------------
function meshgrid(px::LinSpace{Float64},py::LinSpace{Float64})
